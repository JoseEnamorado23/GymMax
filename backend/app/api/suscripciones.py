...
@router.get("/usuario/{usuario_id}", response_model=Optional[SuscripcionResponse])
def obtener_suscripcion_usuario(usuario_id: UUID, db: Session = Depends(get_db)):
    return crud_suscripcion.obtener_ultima_suscripcion(db=db, usuario_id=usuario_id)
